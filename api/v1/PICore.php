...
//Procedimiento para agregar
        try {
            ...
            $stmt = $Db->Sentencia("EXEC proveedor.Ag_core ?, ?, ?");
            $stmt->bindParam(1, $_REQUEST{'nit'});
            $stmt->bindParam(2, $_REQUEST{'nombre'});
            $stmt->bindParam(3, $_REQUEST{'regimen'});
...
//Procedimiento para Eliminar
        try {
            ...
            $stmt = $Db->Sentencia("EXEC [proveedor].[sp_elimcore] ?");
...
//Procedimiento para modificar
        try {
            ...
            $stmt = $Db->Sentencia("EXEC [proveedor].[sp_Modcore] ?, ?, ?");
...
$data = $Db->GetData("select * from proveedor.core", $_SESSION['dbUser'], $_SESSION['dbPass']);
